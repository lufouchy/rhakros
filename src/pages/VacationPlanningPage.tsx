...
import { CalendarDays, List, ChevronLeft, ChevronRight, Palmtree, Loader2, UserX, Stethoscope, Clock } from 'lucide-react';
...
const VacationPlanningPage = () => {
  ...
  const fetchData = async () => {
    ...
    absRes.data?.forEach(a => {
      ...
      // Filter: only include if overlaps with current month
      if (start <= monthEnd && end >= monthStart) {
        result.push({
          ...
          position: p.position,
          type: 'absence',
          subType: a.absence_type || undefined,
          startDate: start,
          endDate: end,
  ...
  return (
    <SidebarLayout>
      <div className="p-4 md:p-6 space-y-6 animate-fade-in">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold text-foreground">Programação de Férias e Ausências</h1>
          <p className="text-muted-foreground text-sm mt-1">Visualize quem está de férias ou ausente no período selecionado.</p>
        </div>
